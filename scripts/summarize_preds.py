...
if __name__ == "__main__":
	...
	with open(args.accurate,"wt",newline="") as accurate_out, open(args.inaccurate,"wt",newline="") as innacurate_out:
		...
		for chunk in args.chunk_results:
			with gzip.open(chunk) as chunk_result:
				for s_result in chunk_result:
					...
					for annotation in s_result:
						sample_total += 1
						in_alts = 1 if annotation["standard"] in annotation["preds"] else 0
						in_alts_total += in_alts
						row = [annotation["observed"], annotation["standard"], annotation["final_pred"][args.layers]["alt"], 
							annotation["final_pred"][args.layers]["CD"], annotation["final_pred"][args.layers]["LD"], int(annotation["final_pred"][args.layers]["acc"])]
		 ...
		 with open(args.summary,"wt") as summary_out:
		 	...
		 	summary_out.write("False and not observed in alts: " + str(inaccurate_not_in_alts) + "\n")
